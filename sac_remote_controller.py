...
📍 目标控制命令:
  x y z          设置新目标点 (例: 0.5 0.5 1.0)
  home           返回起始位置 [0, 0, 0.1]
  stop           立即停止飞行（悬停在当前位置）
  
🎯 快速测试命令:
  test1          前往测试点 1: [0.5, 0.5, 1.0]
  test2          前往测试点 2: [0.8, 0.3, 1.0]
  test3          前往测试点 3: [0.3, 0.8, 1.0]
  test4          前往测试点 4: [-0.5, 0.5, 1.0]
  test5          前往测试点 5: [-0.5, -0.5, 1.0]
  testall        依次访问所有测试点
...
💡 提示:
  - 训练空间范围: X/Y ∈ [-1.5, 1.5], Z ∈ [0.3, 2.0]
  - 建议测试范围: X/Y ∈ [-0.8, 0.8], Z ∈ [0.5, 1.5]
  - 起始高度约 1.0m，建议目标高度在 0.8-1.5m 范围
  - 每个命令发送一个目标，无人机按顺序导航
    ...
    def parse_command(self, user_input: str) -> Optional[Dict[str, Any]]:
        ...
        # 预定义任务
            ...
            for i in range(8):
                ...
                z = 0.8 + i * 0.05  # 从 0.8m 逐步上升到 1.15m
